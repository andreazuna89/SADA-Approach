...
class DANN(object):
    ...
    def fit(self, X, Y, Y_domain, X_adapt, Y_valid, Yt_domain, X_valid=None, do_random_init=True):   
        ...
        subj_labels = len(set(Y_domain))+1
        ...
        for t in range(self.maxiter):
            ...
            for i in range(nb_examples):
                ...
                if self.lambda_adapt == 0.:
                    ...
                    gho_x_t_2 = self.softmax(np.dot(U, hidden_layer_2) + d) 
                    
                    delta_d += self.lambda_adapt * (subj_hot - gho_x_t_2) 
                    delta_U += np.dot(delta_d.reshape(-1,1), hidden_layer_2.reshape(1,-1))

                    if self.adversarial_representation:
                        tmp = np.dot(U.T,self.lambda_adapt * (subj_hot - gho_x_t_2))*hidden_layer_2 * (1.-hidden_layer_2)
                        delta_b += tmp
                        delta_W += np.dot(tmp.reshape(-1,1), x_t_2.reshape(1,-1))
